...
| `NETMAN_SendPacket` — **partially FIXED (2026-08-08, native-session-glue-cluster)**: the canonical declaration (`network/Netman.h`) and its `GameConfig::GameConfig()`/`native/NETMAN_SessionSettings.c` call sites are fixed; `ui/EditWindow.cpp` and `native/NETMAN_NetworkUI.c` still carry their own separate mismatched declarations, see "Fixed so far" | `ui/EditWindow.cpp`, `native/NETMAN_NetworkUI.c` | `(unsigned char*)` — `native/NETMAN_SessionSettings.c` |
