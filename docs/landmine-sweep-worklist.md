...
**2026-08-17 update**: the dedicated future pass happened (GameConfig-
singleton-unification session) — `_g_dplay`/`_g_dplay_config` (both a
naming collision with this NetworkPlayerList discussion AND, separately,
dead aliases of the unrelated GameConfig singleton at 0x4FD3A8) are now
removed tree-wide; every remaining reader uses the real `g_dplay`/
`g_dplay_config` (no leading underscore). See `game/GameConfig.h`'s header
comment for the full consolidation.
...
| -- | `NETMAN_SendPacket(void*)` in `ui/EditWindow.cpp` (0x43CDF0 annotation is bogus) | **FIXED (2026-08-17, GameConfig-singleton-unification session).** `ui/EditWindow.cpp` now declares/calls the real `void __fastcall NETMAN_SendPacket(GameConfig* packetPtr);` (network/Netman.h's canonical declaration) with `NETMAN_SendPacket(_g_netman_data)` -- `_g_netman_state` (the `char*` raw-offset view this row was written against) has been removed tree-wide in favor of the one canonical `_g_netman_data` (`GameConfig*`). See `game/GameConfig.h`'s header comment for the full singleton consolidation (also closes the two rows below). |
| -- | `native/NETMAN_NetworkUI.c`'s `NETMAN_FreePacket`/`NETMAN_SendPacket(int32_t)` | **Already resolved before this row was last verified** -- as of 2026-08-17, this file's own local externs already declare the real `(GameConfig*)` shape (`extern void __fastcall NETMAN_SendPacket(GameConfig* packetPtr);`) and its `_g_netman_data` local was already typed `GameConfig*` in the 2026-08-17 singleton-unification pass, not `int32_t`/`void*`. |
| -- | `game/GameConfig.h`'s `extern GameConfig* g_dplayConfig;` -- dangling, never defined | **FIXED (2026-08-17).** Removed; `_g_netman_data` (declared in `game/GameConfig.h`, defined in `shared/defsym_stubs.cpp`, assigned for real by `shared/stubs_impl.cpp`'s `GameConfig_constructor` via `core/GameLoop.cpp`'s `GameLoop_Setup`) is now the one canonical, actually-constructed pointer for this singleton -- construction was also fixed in the same pass (it previously placement-constructed a host-only `DPlayConfig` byte-buffer stand-in instead of a real `GameConfig`). |
...
| 2 | `AssetMgr_ReadPairValue` — **FIXED (2026-08-08, Building.cpp STRICT=2 cast cluster; further corrected 2026-08-17)** | AssetMgr_ReadPairValue(AssetMgr*, unsigned int, unsigned int) | Building::StepToward(int, int), Building::FindNearestConnectionNode(void*, unsigned int) — both call sites now static_cast their `void*`/AssetMgr-shaped pointer to `AssetMgr*` (forward-declared locally in Building.cpp; resources/AssetMgr.h itself is deliberately NOT included there — see that file's top-of-file comment on why). **2026-08-17 correction:** the 2026-08-08 fix cast the right *type* but the wrong *global* for StepToward's call — it used `g_asset_mgr` (0x485600, the unrelated file-archive `AssetArchive` singleton, see resources/AssetArchive.h) instead of the real `this` pointer, which Ghidra disassembly of Building_StepToward (0x432AE0) proves is loaded from a distinct global at 0x4FD190. Fixed to a new, correctly-addressed `g_tile_adjacency_mgr` (0x4FD190); FindNearestConnectionNode was unaffected (it already took its `node_set` parameter directly, never touched either global). Both 0x485600 and 0x4FD190 have zero WRITE xrefs anywhere in the binary, so this call remains dead code in the shipped retail game either way — the fix is for correctness/documentation, not observable behavior. |
...
| 1 | `AssetMgr_LoadFile` — **FIXED (2026-08-17, g_asset_mgr extern-global-type-mismatch cleanup)** | AssetMgr_LoadFile, AssetMgr_LoadFile(int*, unsigned char*, int*), AssetMgr_LoadFile(void*, unsigned char*, int*) | GameSetupPanel::loadLayouts(bool) — plus ~17 other call sites across the tree (ui/CursorEditWindow.cpp, ui/HelpWnd.cpp, ui/UIPANEL_Surface.cpp, game/TrainStation.cpp, network/Netman.cpp, input/Cursor.cpp, input/TrackTileDescriptor.cpp, input/BuildingDescriptorEditor.cpp, resources/ResDataSave.cpp, native/cgwnd_palette.c, native/wave_io.c). Root cause: the underlying global `g_asset_mgr` (0x485600) was extern-redeclared with ~6 mutually incompatible types (void*/AssetMgr*/int32_t/uint8_t) across the tree instead of from one canonical header, so each call site's mangled symbol bound to a different (mostly empty no-op) overload instead of the real implementation. Ghidra disassembly (RESMGR_OpenResourceFile @ 0x448A70, Train_DownloadMissingAssets @ 0x438E40) proves `g_asset_mgr` is a VALUE object (`this` = the literal address `0x485600`, not `[0x485600]`), not a pointer of any kind — resolved as `AssetArchive g_asset_mgr;` (resources/AssetArchive.h/.cpp), a class distinct from resources/AssetMgr.h's unrelated `AssetMgr` (a Ghidra auto-naming collision — see AssetArchive.h's header comment). Also uncovered and fixed in the same pass: game/Building.cpp's Building::StepToward was reading a *different*, unrelated global (0x4FD190, now `g_tile_adjacency_mgr`) but had been made to alias `g_asset_mgr` by the 2026-08-08 fix below. `native/assetmgr_loadfile.c` retired (folded into resources/AssetArchive.cpp); orphaned no-op stub overloads removed from shared/core_stubs.cpp and shared/link_stubs.cpp. |
...
| `AssetMgr_LoadFile` — **FIXED (2026-08-17)** | `game/TrainStation.cpp`, `input/BuildingDescriptorEditor.cpp`, `ui/HelpWnd.cpp`, and ~14 other call sites | Root cause was the `g_asset_mgr` global itself, not just this function — see the dedicated row above (first table) for the full writeup. Real implementation moved to `resources/AssetArchive.cpp` (`AssetArchive::LoadFile`); `native/assetmgr_loadfile.c` retired. |
