 ...
 *                     +0x08 heap buffer (0x28 bytes, 10 slots)
 ...
 *   3. ResetWorldState (0x41E100) clears the list and zeroes sub-counts
 *      (with an empty g_game, so no Game object is needed)
 *   4. DtorBody (0x41D2D0) frees the buffer and resets count/capacity
 *
 * All other InputMgr.o symbols (Game::DeselectGameObject, Entity::Update,
 * Entity deletion) are left for the linker to ignore — the exercised paths
 * never reach them (g_game == nullptr and the list is empty on reset).
 */
...
int main()
{
    ...
    {
        InputMgr mgr;
        int a = 1, b = 2, c = 3;   /* dummy element addresses */
        mgr.buffer[0] = &a;
        mgr.buffer[1] = &b;
        mgr.buffer[2] = &c;
        mgr.count = 3;
        ...
        void* removed = mgr.ListRemoveAt(1);
    ...
    /* ---- 3. ResetWorldState (0x41E100) on an empty list ---- */
    {
        InputMgr mgr;
        mgr.entity_count = 4;
        mgr.special_count = 2;
    ...
    if (failures == 0) {
        std::puts("PASS: canonical InputMgr ctor/collection/reset/dtor contract");
        return 0;
    }
