...
require 'panel.hostSearchRequested = true;' "$panel"
require 'lego_loco::network::HostDiscoveryWorker().Start({true, publication});' "$panel"
require 'if (discovery.browse_ready)' "$panel"
require 'loco::host_test::emit_search_completed(' "$panel"
require 'bool hostSearchRequested = false;' "$header"
require 'bool hostSearchEventEmitted = false;' "$header"
require 'lego_loco::network::HostTransportWorker().StartHost({' "$panel"
require 'lego_loco::network::HostDiscoveryWorker().Start({false, advertisement});' "$panel"
require 'host_preferred_endpoint(session)' "$panel"
require 'lego_loco::network::HostTransportWorker().Join({' "$panel"
require 'bool hostDiscoveryPublished = false;' "$header"
require 'bool hostJoinRequested = false;' "$header"
