/**
 * NetmanTypes.h — Netman's data types (PlayerSlot, PingEntry, InboundTrainNode,
 * CarObject/SpriteObject handles) plus the `class Netman` declaration itself.
 *
 * Lego Loco (loco.exe, 1998, MSVC x86)
 * Reverse engineered via Ghidra decompilation.
 *
 * Split out of Netman.h for the same reason network/PostBagFileNode.h was
 * split out of NetworkPlayerList.h/Netman.h (see that file's header comment):
 * translation units that only need the complete `Netman`/`PlayerSlot` types
 * for named-field member access (`g_netman->m_gameMode`,
 * `slot->msg_queue`, ...) shouldn't have to pull in Netman.h's much larger
 * surface — its extern "C" Win32 API block and ~13 free-function
 * declarations (Vehicle_Ctor, DPLAY_CreatePlayer, Config_GetIniInt, etc.)
 * that Netman.h gives **C++ linkage**.
 *
 * That C++-linkage choice for those 13 names is a pre-existing latent bug
 * in Netman.h, not a style preference: the real, working implementations of
 * those functions are compiled with **C linkage** (confirmed via `nm` on
 * the linked binary — plain unmangled symbols like `T Vehicle_Ctor`), while
 * the mangled C++ symbols Netman.h's declarations would resolve to
 * (`_Z12Vehicle_CtorPviicc` etc.) are separate, already-defined **host
 * no-op stubs** (`shared/defsym_stubs.cpp`, `shared/link_stubs.cpp`) used
 * today only by the handful of TUs (Netman.cpp, Town.cpp) that already
 * include full Netman.h and therefore already silently call the stub, not
 * the real function, for those 13 names. Any TU (like game/Train_network.cpp)
 * that already has its own correct `extern "C"` declarations for these
 * names must keep using them — do NOT delete those in favor of
 * `#include "Netman.h"`, or every one of those 13 calls silently repoints
 * from the real implementation to a no-op stub. Fixing Netman.h's linkage
 * for those 13 declarations is tracked separately as a future dedicated-
 * session cleanup (tree-wide change to a canonical header); this split only
 * avoids the *collision*, it does not fix the underlying mislinkage.
 *
 * Real Netman method implementations: network/Netman.cpp.
 *
