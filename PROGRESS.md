...
| 2026-08-03 (mode3-town-bootstrap) | Validated g_town_view (0x4852A0) and g_ddraw_building (0x4A9EF0) are BSS-embedded GameView/DDRAW_Building objects in the original binary (Ghidra evidence: GameLoop_FrameUpdate passes &g_town_view, CGWND_Cleanup calls GameView_Cleanup(&g_town_view)). Created heap-allocated host equivalents via loco::host::BootstrapTownMode3Objects(). Transcribed Town_TrackBuilding (0x42D1A0, 72 insns) and DDRAW_UpdateBuilding (0x459DA0, 307 insns) from Ghidra disassembly into sdl3_shims/sdl3_town_mode3.cpp. Removed null guards in GameLoop_FrameUpdate; replaced empty stubs in stubs_impl.cpp with extern declarations. Added sdl3_town_mode3.cpp to Makefile SHIM_SRCS. The per-frame mode-3 callbacks now execute real code on non-null objects (both early-return on zero guard bytes for fresh objects — safe no-ops until Town::show() populates state). Next: tile data loading from world save, Town::show() initialization, DDRAW surface wiring. |
...
| 2026-08-02 (town-overlay-cone) | Constructed all 8 Town overlay cone subsystems (Town, Cursor, PostcardAlbum, PostcardPreviewWindow, TrainStationWindow, HelpWnd/AudioMgr, AboutDialog) in `CGWND::InitAllSubsystems` host path. Fixed `WIN32_StreamOpenFile` and `WNDPROC_StreamFromMemory` stubs (void→pointer return). Guarded `Cursor::init()` and `HelpWnd::init()` behind `#ifdef _WIN32` (heavy file I/O not yet host-ready). Moved `TrainStationWindow.h` and `AboutDialog.h` to unconditional includes. `initMode1` PATH A incremental inits now execute; 27/27 tests pass, 136/135 objects build. |


[Showing lines 1-403 of 545 (50.0KB limit). Use offset=404 to continue.]