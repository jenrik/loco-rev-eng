...
| 2026-07-29 (mode2-scenario-suite) | Added isolated Wayland tests for the requested Back/Exit, single-player Go, default OS-name multiplayer, and Multiple→Host→Go→Back→Back flows. Passive events now prove queued click WAVs and accepted names; repaired GameLoop's stale void-pointer PlayerRecord constructor dispatch so Ghidra's OS-name fallback executes. `make test-all` passes. |
| 2026-07-29 (singleplayer-go-routing) | Corrected selector semantics from Ghidra plus RT_STRING evidence: `0x407/0x408` are singleup/singledown and set DPlayConfig+7; `0x409/0x40A` select multiplayer and clear it. Host Go now sends selected single player only to state 4, never the multiplayer lobby; a new isolated GUI regression proves the route. |
| 2026-07-29 (mode2-main-menu-button-feedback) | Recovered the unlabelled main-menu callback block at `0x422930..0x422D6F` from Ghidra-backed bytes. The SDL host now applies the original left/right DPlayConfig+7 selector toggle (so multiplayer visibly pushes from 0x407 to 0x408), its mode changes/click WAV, and delayed 0x404/0x406 accept/quit artwork; all tests pass. |
| 2026-07-29 (mode2-lobby-press-feedback) | Validated `GAMESTATE_HandleClick` and `Sprite_SetState` in Ghidra: all actionable lobby buttons/valid selections play 0x5015, use horizontal sprite frame 1, then sleep 0x96. SDL now retains the pressed frame for 150 ms, plays the archive WAV, and defers Exit/Search/Options accordingly; full test suite passes. |
| 2026-07-29 (multiplayer-selector-regression) | Compared against a clean, isolated build of known-good commit 105dcd6 after the right-hand control regressed. Restored the original 0x422010 state-7/provider-list artwork and hit-test gates (including its 0x409 multipleup branch), retaining only the host-only selection-to-state-5 handoff. Static plus isolated-Wayland regressions and `make test-all` pass. |
| 2026-07-29 (multiplayer-entry-selection) | Fixed the SDL menu path that could only enter state-4 setup while rendering it as a network lobby: host acceptance enters state 5 after the explicit multiplayer choice, and the guarded compositor labels state 4/5 correctly. Full deterministic + isolated-Wayland suites pass; artifacts prove selection → state-5 lobby → empty search → exit. |
| 2026-07-29 (exit-audio-stall) | **Fixed mode-10 exit audio stall**: CGWND_sdl3.cpp PumpMessages_SDL3 incorrectly waited for exit sound (0x5026) to fully drain before exiting. Ghidra confirms AudioChannel_IsActive (0x40EEB0) only checks if playback started, not finished. Original code at 0x408276 posts WM_CLOSE immediately after IsActive returns true. Removed SDL3_GameAudioPump() drain loop from mode-10 path — just exits directly. Both exit-control and escape-focused tests pass (was: 5.2s stall, now: instant). |
