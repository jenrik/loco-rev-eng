...
| 2026-08-01 (cgwnd-entermode3) | Implemented `CGWND_EnterMode3` at 0x4086F0 instruction-by-instruction from Ghidra: 9-case switch on old_mode. Case 2 cancels to menu. Cases 5/6/7/9 hide overlay subsystems via vtable[1], update audio, set screen mode, invalidate tilemap rect. Case 1 toggles fullscreen, kills timer, sends netman data, posts WM_USER+6, checks demo audio mute, updates tilemap, plays narration, then falls through to case 4. Case 4 posts WM_USER+6, sends netman data, sets screen mode, processes object timers, conditionally updates tilemap/buildings. Common tail invalidates buildings, resets tooltips/world, queues async task at 0x42CC60, clears build-mode state, dispatches cursor feedback, silences audio. Removed two matching defsym stubs (`void CGWND_EnterMode3(int){}` and `void CGWND_EnterMode3(){}`). 74/74 `make check`, default build links, 8/8 component tests pass (1 pre-existing archive-data failure). |
