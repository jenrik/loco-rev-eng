...
- [ ] **ui_childwindow.c removed** — `UI_CreateChildWindow`/`UI_ChildWindow_Dtor`/`UI_PaintWindow`/`UI_OnMouseLeave`/`UI_IsBitmapReady` (0x424AF0–0x4257F0) are called live from CursorEditWindow.cpp/TrainStation.cpp/UIPANEL.cpp/scriptengine.cpp/Town.cpp but backed only by silent no-ops.
...
- [ ] **win32_network.c removed** — `WIN32_SendNetworkData`/`RecvNetworkData`/`GetSystemMetrics`/`PeekMessageLoop` (0x460360–0x460EA0) called from Train_network.cpp/DirectPlay.cpp; real DirectPlay marshalling was never carried over.
- [ ] **win32_stream.c removed** — stream I/O functions (0x463810–0x461880) called from a dozen+ sites (ResourceManager.cpp, ResDataSave.cpp, UIPANEL_Surface.cpp, GameSetupPanel.cpp, Cursor.cpp); flagged HIGH priority previously (see below), still unresolved.
...
- [ ] **Restore the current GUI integration gate** — `nix develop -c meson test -C build --suite integration` now reports 2 failed / 10 passed (test_singleplayer_accept_reaches_mode3_without_crashing, test_main_menu_escape_exits_cleanly), up from the previously-tracked 8 failed / 4 passed — the 2026-08-05 Make→Meson source-tree unification linked network/discovery shim code into the real binary for the first time (previously compiled only into standalone test binaries), fixing several multiplayer-flow failures as a side effect; preserve artifacts under `build/test-artifacts/` while diagnosing the remaining 2.
