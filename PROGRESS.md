...
- [x] **Mode-2 multiplayer entry selection** — Separated SDL-only selection from persisted DirectPlay configuration. PE resource evidence identifies `0x407/0x408` as singleup/singledown and `0x409/0x40A` as multipleup/multipledown; the host renders both choices and the right-hand `0x409` selection enters state 5 after name acceptance. The GameSetup compositor labels state 4 as scenario setup and state 5 as Network Game; isolated-Wayland coverage captures selection → state-5 lobby → empty search → exit.
...
| 2026-07-29 (multiplayer-selector-regression) | Corrected the host mapping after the right-hand multiplayer button disappeared: PE strings prove 0x407/0x408 are singleup/singledown and 0x409/0x40A are multipleup/multipledown. The host now always composes both choices, maps 0x409 to state 5, and has static plus isolated-Wayland regression coverage. `make test-all` passes. |
