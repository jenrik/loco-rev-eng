...
- [ ] **Complete gameplay object/file integration** — Non-empty `0x3EC` records now become grouped `Vehicle`/`VehicleEditor` objects and enter Netman’s canonical native-width list; `0x3EE` assets replace by key; and `0x3FC` streams assemble exactly. Migrate residual mode-3 Vehicle/editor serialization away from raw original offsets, connect stored assets to concrete consumers, and validate movement/train transfer plus backpressure in mode 3.
