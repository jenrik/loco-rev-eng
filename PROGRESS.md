...
- [ ] **InputMgr cone + world load** — construct g_input_mgr (InputMgr, 0x4A9990; ctor InputMgr::InputMgr(HINSTANCE,UINT) at 0x41F5E0-area; the original CRT thunk used a no-arg ctor at ~0x41D31A; check which fields the loader touches). native/input_world.c (INPUT_NewWorld 0x41E120 / INPUT_LoadWorld 0x41D320 / INPUT_LoadSaveFile 0x41D5C0) is currently in NATIVE_BROKEN — port it to C++ or enable it. Wire the host loading task (loco::host::HostLoadingSequence in HostMode3Bootstrap.cpp, currently logs "world-file load deferred") to call INPUT_NewWorld for a fresh SP game and INPUT_LoadWorld("~curr") per the original 0x45DE40 worker. lego-loco-unpacked ships no .loco saves, so a new game must create one.
...
- [ ] **Mode-3 frame deps** — g_town_view (0x4852A0), g_ddraw_building (0x4A9EF0): construct + wire Town_TrackBuilding / DDRAW_UpdateBuilding; remove the GameLoop_FrameUpdate mode-3 null-guards. g_input_mgr guards resolve with the InputMgr cone.
