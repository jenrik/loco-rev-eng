...
- [ ] **Host mode-3 bootstrap** — `Town`, `Cursor`, `PostcardAlbum`, `Game`, `BuildingMgr`, `World`, and tilemap are now fully decompiled and INTEGRATED (typed constructors recovered); they remain unconstructed on the SDL host. Mode-1 PATH A and `CGWND_EnterMode3(0x4086F0)` still cannot safely progress to host gameplay until their SDL boundaries are wired in; never fabricate a town frame.
