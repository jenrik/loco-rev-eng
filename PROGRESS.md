...
- [ ] **ui_manager.c removed** — UI-Manager ctor/tooltip/message-box functions (0x4238C0–0x423D20) called from DDRAW.cpp/BuildingMgr.cpp/Game.cpp/ScriptedObject.cpp/tilemap.cpp; no typed reconstruction exists.
...
- [x] **Restore the current GUI integration gate** — was 2 failed / 10 passed after the 2026-08-05 Make→Meson unification, briefly regressed to 3 failed / 9 passed (see the input-edit-cluster entry above) due to an unrelated link gap, now fixed — see "2026-08-05 (link-gap-fix)" below. Current: 1 failed / 11 passed (only the long-tracked `test_singleplayer_accept_reaches_mode3_without_crashing`).
...
- [ ] **Strict regression gate** — Make Tier 2/3 (`meson configure -Dstrict=1|2 build`) forced clean builds and the full test layer green after archive test data is available; current `meson test -C build` stops because `lego-loco-unpacked/art-res/resource.RFH` is absent.
