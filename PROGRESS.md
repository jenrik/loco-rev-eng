...
- [ ] **World-file load (next persistence milestone)** — port native/input_world.c semantics (INPUT_NewWorld 0x41E120 / INPUT_LoadWorld 0x41D320 / INPUT_LoadSaveFile 0x41D5C0 / INPUT_SaveCurrentWorld 0x41D9B0) into C++ over the canonical InputMgr embedded collection; wire loco::host::HostLoadingSequence to call INPUT_NewWorld for a fresh SP game and INPUT_SaveCurrentWorld/INPUT_LoadWorld("~curr") per the original 0x45DE40 worker; lego-loco-unpacked ships no .loco saves, so a new game must create one. The deferred stubs in InputMgr.cpp log and abort until then.
